  ...
  const handleFileChange = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      try {
        ...
        setImages((prevImages) => [imageInfo, ...prevImages]);
      } catch (error) {
    ...
    <div className="w-full h-full flex flex-col items-center justify-center pt-14 px-4 md:px-14 lg:px-32 ">
      <div className="w-full flex flex-col md:flex-row lg:h-[550px] items-center gap-16 md:px-8 py-10">
        <div className="flex flex-col items-start w-full lg:w-[600px]">
          ...
          <button onClick={openFileSelector} className=" bg-blue-500 hover:bg-blue-700 text-white rounded-xl font-bold py-4 px-10  mt-10 text-xl">
            Upload Image
          </button>
        </div>
      ...
      <div className="lg:hidden">
        {images.length > 0 && (
          <div className="mt-8 grid grid-cols-1 md:grid-cols-2 gap-8 py-4">
            {images.map((image, index) => (
              <div
                key={index}
